...
export const ProductProvider = ({ children }) => {
    ...
    const addProduct = async (productData) => {
        ...
        try {
            ...
            formData.append('vehicleName', productData.vehicleBrand); // vehicleBrand -> vehicleName
            ...
            // Add image if provided - safely extract URI
            if (productData.image) {
                ...
                if (imageUri && imageUri.startsWith('file')) {
                    ...
                    console.log("FORM DATA IMAGE ADDED:", imageUri);
    ...
    const updateProduct = async (id, updatedProductData) => {
        ...
        try {
            ...
            formData.append('vehicleName', updatedProductData.vehicleBrand); // vehicleBrand -> vehicleName
            ...
            if (updatedProductData.image) {
                ...
                console.log("IMAGE URI FOR UPDATE:", imageUri);
                if (imageUri && imageUri.startsWith('file')) {
                    const filename = imageUri.split('/').pop();
                    const match = /\.(\w+)$/.exec(filename);
                    const type = match ? `image/${match[1]}` : `image/jpeg`;
