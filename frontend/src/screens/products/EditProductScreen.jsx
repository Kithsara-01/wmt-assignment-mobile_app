...
// Safe helper to resolve image URIs
...
const resolveImageUri = (image) => {
    console.log("RESOLVING IMAGE URI:", image);
    if (!image) return null;
...
const EditProductScreen = ({ route, navigation }) => {
    ...
    const handleUpdate = async () => {
        ...
        // 4. Update in Context (now calls API)
            ...
            <View style={styles.formCard}>
                <TouchableOpacity style={styles.imagePicker} onPress={pickImage}>
                    {image && image.uri ? (
                        ...
                        <Image source={{ uri: resolveImageUri(product.image) }} style={styles.previewImage} />
                    ) : (
